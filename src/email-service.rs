...
/// DuckMail 临时邮箱提供商（https://api.duckmail.sbs）
///
/// 支持 `generate_email` 自动创建临时邮箱，每个邮箱使用独立的 Bearer token。
pub struct DuckMailProvider {
    api_base: String,
    api_key: String,
    default_password: String,
    domains: Vec<String>,
    /// per-email token 缓存: email → (bearer_token, password)
    token_cache: Mutex<HashMap<String, (String, String)>>,
    /// 域名缓存（首次 GET /domains 后缓存）
    cached_domains: tokio::sync::OnceCell<Vec<String>>,
    clients: Mutex<HashMap<String, rquest::Client>>,
}

impl std::fmt::Debug for DuckMailProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DuckMailProvider")
...
impl DuckMailProvider {
    pub fn new(
        ...
        Self {
            api_base: "https://api.duckmail.sbs".to_string(),
            api_key,
            default_password,
            domains,
            token_cache: Mutex::new(HashMap::new()),
            cached_domains: tokio::sync::OnceCell::new(),
            clients: Mutex::new(HashMap::new()),
        }
    }
    ...
    /// 获取可用域名（优先使用配置列表，否则从 API 获取并缓存）
    async fn get_domains(&self, proxy: Option<&str>) -> Result<Vec<String>> {
        ...
        self.cached_domains
            .get_or_try_init(|| async {
                let mut url = format!("{}/domains?page=1", api_base);
                ...
                for _ in 0..3 {
                    ...
                    let resp = req.send().await.context("DuckMail GET /domains 失败")?;
                    if resp.status() != StatusCode::OK {
                        bail!("DuckMail /domains HTTP {}", resp.status());
                    }
                    ...
                    {
                        for m in members {
                            if let Some(d) = m.get("domain").and_then(|v| v.as_str()) {
                    ...
                    let has_next = json
                        .get("hydra:view")
                        .and_then(|v| v.get("hydra:next"))
                        .is_some();
                    if !has_next {
                        break;
                    }
                    let next_page = json["hydra:view"]["hydra:next"]
                        .as_str()
                        .unwrap_or_default();
                    url = format!("{}{}", api_base, next_page);
                }
    ...
    /// 创建临时邮箱账号
    async fn create_email(&self, proxy: Option<&str>) -> Result<(String, String)> {
        let domains = self.get_domains(proxy).await?;
        use rand::Rng;
        let idx = rand::rng().random_range(0..domains.len());
        let domain = &domains[idx];
        let username = crate::util::random_hex(8);
        let address = format!("{username}@{domain}");
        let password = self.default_password.clone();
        ...
        let resp = client
            ...
            .send()
            .await
            .context("DuckMail POST /accounts 失败")?;
    ...
    async fn get_bearer_token(
        ...
        // 检查缓存
        {
            ...
            if let Some((token, _)) = cache.get(email) {
        ...
        let client = self.get_client(proxy)?;
        let resp = client
            .post(format!("{}/token", self.api_base))
            .header("Content-Type", "application/json")
            .json(&serde_json::json!({
                "address": email,
                "password": password
            ...
            .send()
            .await
            .context("DuckMail POST /token 失败")?;

        if resp.status() != StatusCode::OK {
            let body = resp.text().await.unwrap_or_default();
            bail!("DuckMail 认证失败: {}", truncate_text(&body, 200));
        }
        ...
        let token = json
            .get("token")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("DuckMail /token 响应缺少 token 字段"))?
            .to_string();

        // 存入缓存
        let mut cache = self.token_cache.lock().unwrap_or_else(|p| p.into_inner());
        cache.insert(email.to_string(), (token.clone(), password.to_string()));

        Ok(token)
    }

    /// 清除指定邮箱的 token 缓存（用于 401 重试）
    fn invalidate_token(&self, email: &str) {
    ...
    /// 获取缓存中该邮箱的密码
    fn cached_password(&self, email: &str) -> Option<String> {
        self.token_cache
            .lock()
            .ok()
            .and_then(|cache| cache.get(email).map(|(_, pwd)| pwd.clone()))
    }

    /// 将 ISO8601 createdAt 转为毫秒时间戳（作为 email_id）
...
#[async_trait]
impl EmailProvider for DuckMailProvider {
    async fn fetch_emails(
        &self,
        target_email: &str,
        _size: usize,
        ...
        let password = self.cached_password(target_email).unwrap_or_else(|| self.default_password.clone());
        let token = self.get_bearer_token(target_email, &password, proxy).await?;

        let client = self.get_client(proxy)?;
        let resp = client
            .get(format!("{}/messages?page=1", self.api_base))
            .header("Authorization", format!("Bearer {token}"))
            .send()
            .await
            .context("DuckMail GET /messages 失败")?;

        // 401 → 清除 token 缓存并重试一次
        if resp.status() == StatusCode::UNAUTHORIZED {
            self.invalidate_token(target_email);
            let token = self.get_bearer_token(target_email, &password, proxy).await?;
            let resp = client
                .get(format!("{}/messages?page=1", self.api_base))
                .header("Authorization", format!("Bearer {token}"))
                ...
                .await?;
            if resp.status() != StatusCode::OK {
                bail!("DuckMail /messages HTTP {} (重试后)", resp.status());
            }
            return self.parse_messages_response(resp, target_email, &token, proxy).await;
        }

        if resp.status() != StatusCode::OK {
            let body = resp.text().await.unwrap_or_default();
            bail!("DuckMail /messages HTTP 异常: {}", truncate_text(&body, 200));
        }

        self.parse_messages_response(resp, target_email, &token, proxy).await
    }

    async fn generate_email(&self, proxy: Option<&str>) -> Result<Option<String>> {
        let (email, password) = self.create_email(proxy).await?;
        // 预获取 token 并缓存
        self.get_bearer_token(&email, &password, proxy).await?;
        Ok(Some(email))
    }
}

impl DuckMailProvider {
    /// 解析 /messages 列表响应，获取每封邮件的全文
    async fn parse_messages_response(
        ...
        for item in &members {
            ...
            // 仅保留来自 OpenAI 的邮件
            if !from_addr.to_lowercase().contains("openai") {
                continue;
            }
            ...
            let msg_id = item.get("id").and_then(|v| v.as_str()).unwrap_or_default();
            ...
            // 获取邮件全文
            if !msg_id.is_empty() {
                // msg_id 可能是 "/messages/xxx" 或 "xxx"
                let id_part = msg_id.rsplit('/').next().unwrap_or(msg_id);
                let detail_resp = client
                    .get(format!("{}/messages/{}", self.api_base, id_part))
                    .header("Authorization", format!("Bearer {token}"))
                    .send()
                    .await;
                ...
                if let Ok(detail) = detail_resp {
                    if detail.status() == StatusCode::OK {
                        if let Ok(dj) = detail.json::<serde_json::Value>().await {
                            let text = dj
                                .get("text")
                                .and_then(|v| v.as_str())
                                .unwrap_or_default()
        ...
        // 确保按 email_id 降序（newest first），与轮询逻辑一致
        let _ = target_email; // 仅用于 token lookup
        Ok(out)
    }
}
...
impl EmailService {
    ...
    pub fn new_duckmail(
        ...
        Self::with_concurrency(
            Arc::new(DuckMailProvider::new(api_key, password, domains)),
            max_concurrency,
        )
    }
