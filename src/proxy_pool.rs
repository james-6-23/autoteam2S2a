...
impl ProxyPool {
    ...
    /// 是否有任何代理配置了刷新 URL
    pub fn has_refresh_urls(&self) -> bool {
        !self.refresh_urls.lock().unwrap().is_empty()
    }

    /// 同步刷新所有配置了刷新 URL 的代理 IP（await 等待完成）
    /// 用于 owner 级别的批次间刷新，确保新批次拿到新 IP
    /// 内部加锁，多个并发调用方排队执行
    pub async fn refresh_all_and_wait(&self) {
